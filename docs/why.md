...
`go-ruby-thor/thor` reimplements Ruby's `thor` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-thor/thor` directly, with no Ruby runtime;
